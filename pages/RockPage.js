...
export default function RockPage() {
  const { data, isLoading } = useSWR("/api/rockstars", fetcher);
  ...
  return (
    <>
      ...
      <StyledRapLineDiv>
        <StyledRapP>Rock</StyledRapP>
        <StyledHr />
      </StyledRapLineDiv>
      <div>
        <StyledUl>
          {data.map((rockstars) => (
            <StyledLi key={rockstars._id}>
              <StyledImage
                src={rockstars.image}
                width={"550"}
                height={"300"}
              ...
              />
              <StyledRatingP>
                <StyledStarSpan>{rockstars.rating}</StyledStarSpan>
              </StyledRatingP>
              <StyledArtistnameP>{rockstars.name}</StyledArtistnameP>
              <StyledHr2 />
              <StyledPriceP>
                {rockstars.currency} {rockstars.price}
              </StyledPriceP>
              <StyledLink
                href={`/rockDetails/${rockstars._id}`}
                style={{
                  textDecoration: "none",
                  color: "#139df4",
                  position: "relative",
